...
const secretPatterns = [
  ...
  { pattern: /(?:secret|token)\s*=\s*['"][^'"]+['"]/gi, name: 'hardcoded secret/token' },
  { pattern: /(?:mongodb|postgres|mysql):\/\/[^'"]+/gi, name: 'database connection string' },
  { pattern: /-----BEGIN (?:RSA |DSA )?PRIVATE KEY-----/gi, name: 'private key' },
  { pattern: /(?:aws_access_key_id|aws_secret_access_key)\s*=\s*.+/gi, name: 'AWS credentials' }
];

const excludeDirs = ['node_modules', '.git', 'dist', 'dist-prod', 'build', '.vscode'];
